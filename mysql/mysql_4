...
CREATE PROCEDURE ifProc2()
BEGIN
	declare myNum int;		-- 함수 안에서 변수 선언, 함수 밖에서는 @ 사용
    set myNum = 200;
	IF myNum = 100 THEN  
      SELECT '100입니다.';
    ELSE
...
BEGIN
    ...
    DECLARE debutDate1 DATE; -- 데뷰일
    DECLARE curDate DATE; -- 오늘
    DECLARE days INT; -- 활동한 일수
    
	SET debutDate = (SELECT debut_date FROM market_db.member WHERE mem_id = 'APN');
    SET curDATE = CURRENT_DATE(); -- 현재 날짜
    SET days =  DATEDIFF(curDATE, debutDate1); -- 날짜의 차이, 일 단위
...
BEGIN
    ...
    SELECT '1부터 100까지의 합(4의 배수 제외), 1000 넘으면 종료 ==>', i, hap; 
END $$
DELIMITER ;
CALL whileProc2();
